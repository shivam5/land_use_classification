...
def data_augment_rotate (x):
    ...
    # Reshaping the input tensor of shape (no_of_examples, chosen_height*chosen_width*3) 
    # to (no_of_examples, 32, 32, 3) so that it is in proper image format
    shape_x = x.shape
    x = x.reshape(shape_x[0], 3, chosen_height, chosen_width)
    ...
    # to the rasterized format (no_of_examples, chosen_height*chosen_width*3) 
    x = numpy.transpose(x, (0,3,1,2))
    x = x.reshape(shape_x[0], chosen_height*chosen_width*3)
    return x
...
def load_data():
    ...
    # print("Max height is", max(heights))
    # print("Min width is", max(widths))

    # count = 0
    # for i in range(len(heights)):
    #     if (heights[i]>64 or widths[i]>64):
    #         count += 1
    # print(count)
    ...
    if ( os.path.isfile(os.path.join(dataset_folder, "train_data.npy")) ) :
        ...
        i=0
        train_data = np.zeros((1, chosen_height*chosen_width*3+1))
        with open(train_data_label_file) as f:
            content = f.readlines()
            content = [x.strip() for x in content]
            for line in content:
                ...
                if (img.shape[0]<=chosen_height and img.shape[1]<=chosen_height):
                    old_im = Image.open(os.path.join(train_data_images, img_name))
                    old_size = old_im.size

                    new_size = (chosen_height, chosen_width)
                    ...
                    img = cv2.resize(img, (chosen_height, chosen_width)) 


                img = numpy.transpose(img, (2,0,1)).reshape(1, chosen_height*chosen_width*3)
    ...
    if ( os.path.isfile(os.path.join(dataset_folder, "val_data.npy")) ) :
        ...
        i=0
        val_data = np.zeros((1, chosen_height*chosen_width*3+1))
    ...
    if ( os.path.isfile(os.path.join(dataset_folder, "test_data.npy")) ) :
        ...
        i=0
        test_data = np.zeros((1, chosen_height*chosen_width*3+1))
    ...
    basic_training_data_x = train_data[:, 0:chosen_height*chosen_width*3]
    basic_training_data_y = train_data[:, chosen_height*chosen_width*3]
    validation_data_x = val_data[:, 0:chosen_height*chosen_width*3]
    validation_data_y = val_data[:, chosen_height*chosen_width*3]
    testing_data_x = test_data[:, 0:chosen_height*chosen_width*3]
    testing_data_y = test_data[:, chosen_height*chosen_width*3]
    ...
    # training_data_x = training_data[:, 0:chosen_height*chosen_width*3]
    # training_data_y = training_data[:, chosen_height*chosen_width*3]
